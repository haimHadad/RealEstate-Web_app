...
namespace RealEstate_Web_app.Models
{
    public class SalesContract
    {
        ...
        public SalesContract(DappAccount _yourAccount, string contractAddress)
        {
        ...
        public static async Task<string> Deploy(DappAccount yourAccount, uint hoursTimeAmount, uint assetID, string assetLoaction, uint assetRooms, uint assetAreaIn, string assetURL, double assetPrice, string buyerAddress)
        {
            ...
            var estimateGasForDeploy = await yourAccount.Blockchain.Eth.DeployContract.EstimateGasAsync(ABI, ByteCode, yourAccount.Address, contractParams);
            var receiptSalesContract = await yourAccount.Blockchain.Eth.DeployContract.SendRequestAndWaitForReceiptAsync(ABI, ByteCode, yourAccount.Address, estimateGasForDeploy, null, contractParams); 
        ...
        public async Task<bool> sendEtherToContract(double amount) //send money to the contract, if fail return false, else true
        {
            try
            {
                ...
                if (this.getBuyerAddress().Equals(this.accountCaller.Address))
                    return false;
        ...
        public async Task<bool> cancelDeal() //For regulator (=Govrenment)
        {
            ...
            if (!GovrenmentAddress.Equals(accountCaller.Address)) //if the caller is not the regulator
                return false;
            try 
            {
                var cancelContractFunction = deployedContractIsntance.GetFunction("cancelContract");  //find the method of the contract 
                var gasEstimationForCancel = await cancelContractFunction.EstimateGasAsync(accountCaller.Address, null, null);
                var receiptOfCancel = await cancelContractFunction.SendTransactionAndWaitForReceiptAsync(accountCaller.Address, gasEstimationForCancel, null, null);
        ...
        public async Task<bool> approveAndExcecute(double taxPercentage) //for regulator only
        {
            ...
            try 
            {
                ...
                if (buyerSign == true) //check if buyer signed
                {
                    ...
                    var gasEstimationForApproval = await approveAndExcecuteContractFunction.EstimateGasAsync(accountCaller.Address,
                        null, null, new BigInteger(taxEtherAmountAsDouble));
                    var receiptOfApproval = await approveAndExcecuteContractFunction.SendTransactionAndWaitForReceiptAsync(accountCaller.Address,
        ...
        public async Task<bool> denyDeal() //For regulator (=Govrenment)
        {
            ...
            if (!BuyerAddress.Equals(accountCaller.Address)) //if the caller is not the buyer
                return false;
            try
            {
                var denyContractFunction = deployedContractIsntance.GetFunction("denyContract");  //find the method of the contract 
                var gasEstimationForCancel = await denyContractFunction.EstimateGasAsync(accountCaller.Address, null, null);
                var receiptOfDeny = await denyContractFunction.SendTransactionAndWaitForReceiptAsync(accountCaller.Address, gasEstimationForCancel, null, null);
        ...
        public async Task<bool> abort() //in case buyer didnt take action in time
        {
            ...
            if (!SellerAddress.Equals(accountCaller.Address)) //if the caller is not the buyer
                return false;
            ...
            try
            {
                ...
                var gasEstimationForCancel = await abortContractFunction.EstimateGasAsync(accountCaller.Address, null, null);
                var receiptOfAbort = await abortContractFunction.SendTransactionAndWaitForReceiptAsync(accountCaller.Address, gasEstimationForCancel, null, null);
        ...
        public async Task<bool> setIsExpired() //For regulator (=Govrenment)
        {
            ...
            try
            {
                ...
                var gasEstimationForCancel = await setIsExpiredFunction.EstimateGasAsync(accountCaller.Address, null, null);
                var receiptSetIsExpired = await setIsExpiredFunction.SendTransactionAndWaitForReceiptAsync(accountCaller.Address, gasEstimationForCancel, null, null);
